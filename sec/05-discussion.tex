...
\begin{theorem}\label{thm:chlCRLrelation}
  ...
  The there exists a translation function $\mathit{tr}$ such that
  given any statement $P$ in the deterministic fragment of the CHL's imperative language,
  where $P$ is over variables $\vec{x} = x_1,\ldots,x_n$,
  and any first order formulas $\Phi, \Psi$ over variables $\vec{x}_1,\ldots,\vec{x}_k$,
\begin{equation*}
