...
One aspect of the proof system from \Cref{fig:CRLproofsystem} that might be considered suboptimal is that the Conseq rule
refers to the star-extended model. Someone might say that the abstraction provided by the proof system is somewhat ``leaky'',
if the user has to worry about the star-extended model anyway. We answer this objection as follows.
First, the Conseq rule refers only to the star-extended matching logic model, and not to a star-extended reachability system.
Second, in practice one often does not need the full power of the Conseq rule; this rule is typically used to either
propagate a structureless constraint out of or into a component, weaken a component, or weaken the global constraint.
For these use-cases we can provide a collection of rules derived from Conseq that operate on the level of the original matching logic model,
as in \Cref{fig:CRLderivedRules}, and use these instead of Conseq.

\begin{figure}
  \begin{align*}
    & \prftree[l]{ConseqLocal}
      { \prfStackPremises
        { \mathcal{T} \modelsML \varphi \rightarrow \varphi^\prime }
        { (\mathcal{T}, S) \provesCRL [\varphi_1, \ldots, \varphi_{i-1}, \varphi^\prime, \varphi_{i+1}, \ldots, \varphi_k] \land P \Downarrow_{C, E} \Psi^\prime}
      }
      { (\mathcal{T}, S) \provesCRL [\varphi_1, \ldots, \varphi_{i-1}, \varphi, \varphi_{i+1}, \ldots, \varphi_k] \land P \Downarrow_{C, E} \Psi^\prime}
  \end{align*}
  ...
  \begin{align*}
    & \prftree[l]{ConseqGlobal}
      { \prfStackPremises
        ...
        { (\mathcal{T}, S) \provesCRL [\varphi_1, \ldots, \varphi_{i-1}, \varphi, \varphi_{i+1}, \ldots, \varphi_k] \land P^\prime \Downarrow_{C, E} \Psi^\prime}
      }
      { (\mathcal{T}, S) \provesCRL [\varphi_1, \ldots, \varphi_{i-1}, \varphi, \varphi_{i+1}, \ldots, \varphi_k] \land P \Downarrow_{C, E} \Psi^\prime}
  \end{align*}
