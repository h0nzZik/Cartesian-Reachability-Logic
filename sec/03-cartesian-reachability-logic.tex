...
If we restrict configuration lista in CRL claims to contain a single
configuration each, validity of CRL and RL claims neatly coincide.

% TODO: (the proof of the follwong proposition can be found in Appendix~\ref{}):
...
CRL is similar to that of CHL, with only a few minor differences, comming up
with a for CRL is not straightforward.  One could attempt to reuse the proof
...
to support lockstep reasoning about construct with repetitive behaviour: in
order to support \texttt{while} cycles with \texttt{break}, Cartesian Hoare logic itself uses
five additionall fairly complex rules (besides the lifted Hoare logic rules) 
that need to reflect the precise semantics of this construct.  The task for
us is even harder, since we do not know in advance what constructs with
...
Soundness of the proof system, as stated by the follwoing theorem, is the main
