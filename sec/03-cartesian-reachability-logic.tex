...
For these use-cases we can provide a collection of rules derived from rules of \Cref{fig:CRLproofsystem} that operate on the level of the original matching logic model,
as in \Cref{fig:CRLderivedRules}, and use these instead of Conseq (and possibly Reduce).
...
\end{align*}
Now we want to perform case analysis. To do so, we first use the ConseqLocal rule twice to
obtain disjunctions of patterns at the respective components;
then we repeatedly apply the Case rule, leading to four goals;
finally, we use the PropOut rule to propagate the constraints from the components to the global constraint.
...
\item The case in \Cref{ex:case2} represents the situation when the first loop continues execution and the second is finished.
      ...
      We finish this case using the ExFalso rule.
