...
\begin{align*}
  ...
  [ \texttt{if (}X_1 >_{\mathit{Int}} 0 \texttt{)\{y++;x--;}\xi(P, X_1, Y_1) \},  
   \texttt{if (}X_2 >_{\mathit{Int}} 0 \texttt{)\{y++;x--;}\xi(P, X_2, Y_2) \} ] \\
  & \land X_1 \leq_{\mathit{Int}} X_2 \land Y_1 \leq_{\mathit{Int}} Y_2
  \Downarrow_{\Phi_{\emptyset, \mathit{circ}}} \Psi^\prime_{\mathit{mono}} \, ,
\end{align*}
...
\begin{aligned}
  & \mathcal{S}_{\mathit{imp}} \vdash_\CRL
  [ \texttt{if (}{\color{blue}\mathit{true}}\texttt{)\{y++;x--;}\xi(P, X_1, Y_1) \},  
   \texttt{if (}{\color{blue}\mathit{true}}\texttt{)\{y++;x--;}\xi(P, X_2, Y_2) \} ] \\
...
\begin{aligned}
  & \mathcal{S}_{\mathit{imp}} \vdash_\CRL
  [ \texttt{if (}{\color{blue}\mathit{true}}\texttt{)\{y++;x--;}\xi(P, X_1, Y_1) \},  
   \texttt{if (}{\color{blue}\mathit{false}}\texttt{)\{y++;x--;}\xi(P, X_2, Y_2) \} ] \\
...
\begin{aligned}
  & \mathcal{S}_{\mathit{imp}} \vdash_\CRL
  [ \texttt{if (}{\color{blue}\mathit{false}}\texttt{)\{y++;x--;}\xi(P, X_1, Y_1) \},  
   \texttt{if (}{\color{blue}\mathit{true}}\texttt{)\{y++;x--;}\xi(P, X_2, Y_2) \} ] \\
...
\item The case in \Cref{ex:case2} represents the situation when the first loop continue execution and the second is finished.
...
\item The case in \Cref{ex:case1} is the one when we utilize the ability to perform lockstep reasoning.
      We symbolically execute both components until TODO
      %their program parts
