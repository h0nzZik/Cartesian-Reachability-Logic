...
This proof system contains rules\footnote{We have changed the notation
  slightly compared to the original paper.} like\traian{People might be asking ``why?'' about footnote}
...
reachability system and $C$ is introduced below). The proof system is sound and complete:\traian{relatively?} a RL claim is satisfied in $\mathcal{S}$
...
We work only with $\epsilon$-free reachability systems.\traian{Why $\epsilon$-free and not loop-free?}
