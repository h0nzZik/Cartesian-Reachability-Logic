...
\begin{equation}\label{eqn:exampleMLPattern}
    \varphi_{\mathit{example}} \equiv\quad \tap{x--}{\texttt{x}\mapsto X}  \land (X \texttt{ >Int } 1 = \mathit{true})
\end{equation}

% TODO - remark, that in K this would look differently

which, when interpreted in a model of a particular programming language,
denotes the set of program configurations in which the code \texttt{x--} is to be executed
and in which the program variable $\texttt{x}$ has a value $X$ that is greater than $1$.
In this example, the suformula  $\tap{x--}{\mathtt{x}\mapsto X}$
is a nullary term used as a predicate (term-as-predicate), with $X$ being the
only free FOL variable.  Whereas $\texttt{x}$ is not a FOL variable, but a
constant symbol from the signature of the programming language.  The subterm
$\texttt{x}\mapsto X$ states that the program variable $\texttt{x}$ has value
$X$, and the $X \texttt{ >Int } 1 = \mathit{true}$ part then says that the
realization of the function symbol $\_ \texttt{ >Int } \_$ returns the boolean
value $\mathit{true}$ when given $X$ and $1$ as arguments.
...
\begin{equation*}
  \tap{while (x>0) x--}{\mathtt{x}\mapsto V}
  \ \Rightarrow^\exists\  \exists V^\prime.\, \tap{$\cdot$}{\mathtt{x}\mapsto V'} \land (V^\prime \texttt{ <=Int } 0 = \mathit{true})
  \end{equation*}
% \begin{equation*}
%     \begin{aligned}
...
$A, C \vdash_\RL \varphi \Rightarrow^\exists \varphi^\prime$ (where $A$ is a
set of reachability rules and $C$ is introduced below). The proof system is sound and complete: a RL claim is satisfied in $\mathcal{S}$
...
the claim is moved from circularities to rules and can be reused, similarly to the way one assumes a loop invariant
...
\begin{remark}\label{rem:shapeOfReachabilityRules}
  Following the lead of the original reachability logic papers (\cite{RosuS12oopsla,StefanescuCMMSR19}),
...
where $\phi,\phi^\prime$ are terms-as-predicates, and $P,P^\prime$ contain no terms-as-predicate.
  ...
  Thus, with this restriction we support all the reachability systems supported by reachability logic.
\end{remark}
