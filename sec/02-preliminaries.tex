...
This lockstep reasoning is a powerful tool, because the invariants (relating
different executions) required are often very simple. For example consider the
program $P(x,y)$ and the formula \ref{eq:mono_formula} above. If one chooses
the invariant $I$ to be the same as precondition $x_1 \leq x_2 \land y_1 \leq y_2$,
the precondition of the second premise becomes
$x_1 \leq x_2 \land y_1 \leq y_2 \land \neg (x_2 > 0)$, which implies
...
which, when interpreted in a model of a particular programming language,
denotes the set of program configurations in which ``\texttt{x--;}'' is the code
to be executed next and the program variable $\texttt{x}$ has a value $X$ that
is greater than $1$.  In this example, the suformula
$\tap{x--;}{\mathtt{x}\mapsto X}$ is a nullary term used as a predicate
(term-as-predicate), with $X$ being the only free FOL variable.  Whereas
$\texttt{x}$ is not a FOL variable, but a constant symbol from the signature
of the programming language.  The subterm $\texttt{x}\mapsto X$ states that
the program variable $\texttt{x}$ has value $X$, and the
...
made (by means of other rules, essentially performing a symbolic execution),
the claim is moved from circularities to $\mathcal{A}$ (using the
...
again.  We refer an interested reader to~\cite{RosuS12oopsla} for more
details.
...
\begin{remark}\label{rem:shapeOfReachabilityRules}
  Following the original reachability logic papers (\cite{RosuS12oopsla,StefanescuCMMSR19}),
