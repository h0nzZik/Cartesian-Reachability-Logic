...
In order to have relative (with respect to an oracle for deciding matching logic validity) completeness of our proof system, we need to
be able to construct an oracle for deciding validity in the star-extended matching logic model;
the constructed oracle is then used by the relative completeness proof of reachability logic.
Our oracle for deciding validity in the star-extended matching logic model reduces the problem of validity of a matching logic formula
over the extended signature to the problem of matching logic validity of a formula over the original signature in the original model,
for which we have assumed to have an oracle.
In other words, we lift the given oracle for a model $\mathcal{T}$ into an oracle for the model $\mathcal{T}^*$ by means of a reduction function $\Theta$.
...
\begin{itemize}
    ...
    \item the symbols $<$,$+$,$-$,$*$,$\%$ as the usual ordering, addition, subtraction, multiplication, remainder on natural numbers.
\end{itemize}
We represent Gödel's $\beta$ function as e.g., in \Cref{mendelson1997introduction} - as a formula $\beta_P(a,b,c,d)$.
...
\end{theorem}
\begin{proof}
Intuitively, we perform Gödelization of the formula $\varphi^\square$, using Gödel's $\beta_P$ predicate.
...
\begin{itemize}
    \item $\mathit{lookup}(M, l, n, y) = \beta_P(a_{l}, b_{l}, n, \alpha(y))$ whenever $(l, a_{l}, b_{l}) \in M$;
...
\end{itemize}
where $\mathit{valid}(a,b) = \exists l:\mathit{Nat}.\, \forall i:\mathit{Nat}.\, i < l \leftrightarrow \exists y:\mathit{Nat}.\, \beta_p(a,b,i,y)$.
...
properties of the Godel $\beta_P$ predicate and properties of lists in the extended model $\mathcal{T}^*$.
