...
\begin{assumption}\label{a:sf}
    ...
    \begin{equation*}
        \mathit{tr}_{\mathit{con}}(P, \sigma) \Rightarrow^*_{S_{L_{\mathit{CHL}}}} \mathit{tr}_{\mathit{end}}(P, \sigma^\prime)
        \iff
...
\begin{lemma}[Symbolic and concrete match]\label{lem:symConcreteTranslationMatch}
    For any statements $P,Q$ and any injective function $f$ on program variables of $L_{\mathit{CHL}}$,
    the following holds:
    \begin{equation*}
        (\mathit{tr}_{\mathit{con}}(Q, \sigma[f]), \rho) \vDash_{\ML} \mathit{tr}_{\mathit{sym}}(P, f)
        \iff
        \forall j \in \{ 1, \ldots, n \}.\, \rho(\mathit{var}_{\mathit{inj}}(f(x_j))) = (\sigma[f])(x_j)
        \land Q = P
    \end{equation*}
    and
    \begin{equation*}
        (\mathit{end}_{\mathit{con}}(Q, \sigma[f]), \rho) \vDash_{\ML} \mathit{end}_{\mathit{sym}}(P, f)
        \iff
        \forall j \in \{ 1, \ldots, n \}.\, \rho(\mathit{var}_{\mathit{inj}}(f(x_j))) = (\sigma[f])(x_j)
        \land Q = P
    \end{equation*}
...
\begin{lemma}[All-path CRL vs CHL]\label{thm:chlAPCRLrelation}
    ...
    \begin{equation*}
        ...
        \Rightarrow^{c\forall} [\mathit{end}_{\mathit{sym}}(P, r_1),\ldots,\mathit{end}_{\mathit{sym}}(P, r_k)] \land \psi \, .
    \end{equation*}
\end{lemma}
\begin{proof}
    The left side is true iff (by definition)
    \begin{proofenv}
        for every set of valuation pars $\{ (\sigma_1,\sigma_1^\prime),\ldots,(\sigma_k,\sigma_k^\prime) \}$
        satisfying
        \begin{equation*}
    ...
    \begin{proofenv}
        ...
        \begin{equation*}
            \forall P.\, \forall i \in \{ 1, \ldots, k \} . \,
            (\mathit{tr}_{\mathit{con}}(P, \sigma_i[r_i]), \rho_{\sigma^\prime})
            \vDash_{\ML} \mathit{tr}_{\mathit{sym}}(P, r_i) \land \psi
        \end{equation*}
