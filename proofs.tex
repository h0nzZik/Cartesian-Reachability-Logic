...
We assume that the model $\mathcal{T}$ interprets the symbol $\alpha$ as an injective function from configurations into integers.
\begin{theorem}\label{thm:oracleLifting}
For every $\Sigma$-model $\mathcal{T}$
satisfying the conditions above (TODO)
...
Define a function $\mathit{tr}$ which performs basic recursion on a given FOL $\Sigma^*$ formula,
...
\begin{itemize}
    ...
    \item $\mathit{tr}(M, l_1 = l_2) = \forall c,d:\mathit{Cfg}.\, (\beta(a_{l_1}, b_{l_1}, \alpha(c), \alpha(d)) \leftrightarrow \beta(a_{l_2}, b_{l_2}, \alpha(c), \alpha(d)))$ whenever $(l_1, a_{l_1}, b_{l_1}) \in M$
    and $(l_2, a_{l_2}, b_{l_2}) \in M$
    \item TODO handle constants of sort $\mathit{Cfg}^*$
\end{itemize}
