...
\begin{lemma}[One-path / All-path CRL correspondence]
...
\begin{assumption}
    ...
    $S_{L_{\mathit{CHL}}}$ that for any $P,\sigma$ satisfies
    \begin{equation*}
        \mathit{tr}_{\mathit{con}}(P, \sigma) \Rightarrow^*_{S_{L_{\mathit{CHL}}}} \mathit{tr}_{\mathit{concr}}(\texttt{skip}, \sigma^\prime)
        \iff
        \sigma, S \Downarrow \sigma^\prime
    \end{equation*}
    (where $\Downarrow$ denotes the relation defined by the original big-step semantics of $L_{\mathit{CHL}}$).
\end{assumption}

\begin{definition}
    Let $f_{\mathit{inj}}$ be an injective function from program variables of $L_{\mathit{CHL}}$
    into matching logic variables of $\Sigma_{L_{\mathit{CHL}}}$.
    We define a function $\mathit{tr}_{\mathit{sym}}$ by
    \begin{equation*}
        \mathit{tr}_{\mathit{sym}}(P, f_{\mathit{inj}}) \quad \equiv \quad
        \impConfig{P}{
            \mathit{sym}_\mathit{var}(x_1) \mapsto f_{\mathit{inj}}(x_1),
            \ldots,
            \mathit{sym}_\mathit{var}(x_n) \mapsto f_{\mathit{inj}}(x_n)
...
\begin{lemma}[Symbolic and concrete match]\label{lem:symConcreteTranslationMatch}
    For any statement $P$ and any injective function $f$ from program variables of $L_{\mathit{CHL}}$
    into matching logic variables of $\Sigma_{L_{\mathit{CHL}}}$, the following holds:
    \begin{equation*}
        (\mathit{tr}_{\mathit{con}}(P, \sigma[f]), \rho) \vDash_{\ML} \mathit{tr}_{\mathit{sym}}(P, f)
        \iff
        \forall j \in \{ 1, \ldots, n \}.\, \rho(f(x_j)) = (\sigma[f])(x_j)
    \end{equation*}
    (where $x_1,\ldots,x_n$ are program variables occurring in $P$).
\end{lemma}
\begin{proof}
...
\begin{lemma}[Static reasoning in CHL vs CRL]
    ...
    Let $\rho_\sigma$ be a matching logic valuation sending matching logic variables $x$
    to $\sigma(x)$, whenever defined.
    Then:
    \begin{equation*}
        \sigma \vDash_{\FOL} \varphi
        \iff \forall P.\, \forall i \in \{ 1, \ldots, k \} . \,
        (\mathit{tr}_{\mathit{con}}(P, \sigma_i[r_i]), \rho_\sigma) \vDash_{\ML} \mathit{tr}_{\mathit{sym}}(P, f_{r_i}) \land \varphi
    \end{equation*}
\end{lemma}
\begin{proof}
    By the disjointness of the union, the left side is equivalent to
    \begin{equation*}
        \forall i \in \{ 1, \ldots, k \} .\, \sigma_i[r_i] \vDash_{\FOL} \varphi
    \end{equation*}
    which is, since $\varphi$ can be thought of as a structureless ML formula, equivalent to
    \begin{equation*}
        \forall i \in \{ 1, \ldots, k \} .\, \sigma_i[r_i] \vDash_{\ML} \varphi
    \end{equation*}
    which is by \Cref{lem:symConcreteTranslationMatch} equivalent to
    \begin{equation*}
        \forall P.\, \forall i \in \{ 1, \ldots, k \} . \,
        ((\mathit{tr}_{\mathit{con}}(P, \sigma_i[r_i]), \sigma_i) \vDash_{\ML} \mathit{tr}_{\mathit{sym}}(P, r_i))
        \mbox{ and } (\sigma_i[r] \vDash_{\ML} \varphi)
    \end{equation*}
    which is equivalent to the right side by properties of matching logic conjunction and structureless formulas.
\end{proof}
