...
\begin{itemize}
    ...
    \item For the "only if" implication, we assume $C \Rightarrow^*_{\mathcal{S}^*} C^\prime$,
          ...
          Let $i_1,\ldots,i_m \in \{ 1,\ldots,l-1 \}$ be a monotone sequence of maximal length such that
          $C_{i_j}[i] \not = C_{i_j + 1}[i]$ for any $j \in \{ 1,\ldots,m \}$.
          Then clearly, $C_1[j] = C_{i_1}[j]$ (otherwise we could create a longer sequence).
          Similarly, $C_l[j] = C_{i_m}[j]$.
          Now we claim that $C_{i_1}[j] \Rightarrow_{\mathcal{S}} \ldots \Rightarrow_{\mathcal{S}} C_{i_m}[j]$,
          from which the conclusion easily follows.
          We have to prove that for any $o \in \{ 1, \ldots, m \}$, it holds that
          $C_{i_o}[j] \Rightarrow_{\mathcal{S}} C_{i_{o+1}}[j]$.
          But from the definition of the sequence it follows that
          $C_{i_o}[j] = C_{i_{o+1}-1}[j]$, and therefore it is enough to show that
          $C_{i_{o+1}-1}[j] \Rightarrow_{\mathcal{S}} C_{i_{o+1}}[j]$.
          ...
          $C_{i_{o+1}-1} \Rightarrow_{\mathcal{S}^*} C_{i_{o+1}}$, but that is trivial and we are done.
\end{itemize}
\end{proof}
