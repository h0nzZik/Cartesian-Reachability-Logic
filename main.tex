...
\begin{lemma}
    \begin{equation*}
        \mathcal{S} \vdash_\CRL \psi \Downarrow_{C} E
        \implies
        \mathcal{S}^* \vdash_\RL \mathit{flatten}(\psi) \Rightarrow^\forall \bigvee  \mathit{flatten}(E)
    \end{equation*}
\end{lemma}
\begin{proof}
By induction on the structure of the CRL proof. TODO
%\begin{enumerate}
    %\item If the proof ends with \emph{Reduce}, then we use its premise and apply the completeness of all-path reacability logic.
    %\item If the proof ends with \emph{Axiom}, then $\psi \in E$,
    %      and we need to prove $\vdash_\RL \mathit{flatten}(\psi) \Rightarrow^\forall \bigvee  \mathit{flatten}(E)$.
    %      We know that $\mathit{flatten}(\psi) \in \mathit{flatten}(E)$.
%\end{enumerate}
\end{proof}
