...
\author{Jan Tu\v{s}il \and Xiaohong Chen \and Traian Florin Serbanuta
%\and Jan Obdrzalek
}
\date{\today}
...
\begin{enumerate}
    ...
    \item Let $(\Sigma, \mathit{Var})$ be a matching logic signature, and let $\rho$ be a $\mathcal{T}$-valuation.
...
\begin{itemize}
    \item $\varphi \equiv \pi$, where $\pi$ is a basic pattern (of sort $s$).
    \item $\varphi \equiv \varphi_1 \land \varphi_2$.
    \item $\varphi \equiv \neg \varphi^\prime$.
    \item $\varphi \equiv \exists x : s.\, \varphi^\prime$.
\end{itemize}
