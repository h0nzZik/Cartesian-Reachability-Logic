...
\begin{enumerate}
    ...
    \item Let $\mathcal{T}$ be a $\mathbf{\Sigma}$ algebra, and $f$ be a symbol from $\Sigma$.
    ...
    \item A \emph{basic $\mathbf{\Sigma}$-pattern} of sort $s$
          is a term $\phi \in T_{\Sigma, s}(\mathit{Var})$,
          where $\mathbf{\Sigma} = (\Sigma, \mathit{Var})$.
    \item A matching logic $\mathbf{\Sigma}$-formula (aka $\mathbf{\Sigma}$-\emph{pattern})
          of sort $s$
          is a FOL formula where symbols from $\Sigma$ are function symbols
          and basic patterns of sort $s$ are nullary predicate symbols.
    ...
    \item A matching logic \emph{semantics} is given by means of
          ...
          \begin{itemize}
              \item $(\mathcal{T}, \gamma, \rho) \vDash \pi$ iff $\gamma = \rho(\pi)$, where $\pi$ is a basic pattern;
...
\begin{definition}
A \emph{constrained pattern} is a pattern of the form $\phi \land P$,
where $\phi$ is a basic pattern and $P$ a patternless FOL formula.
\end{definition}

\begin{remark}
For simplicity, we restrict the class of reachability systems we work with to those whose reachability rules
have only disjunctions of constrained patterns. That is, we assume all rules have the shape
\begin{equation*}
    \bigvee_{i=1}^{m} (\phi_i \land P_i) \Rightarrow^\exists \bigvee_{j=1}^{n} (\phi^\prime_j \land P^\prime_j) \, .
\end{equation*}
\end{remark}
