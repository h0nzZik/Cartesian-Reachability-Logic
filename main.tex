...
\begin{itemize}
    \item A \emph{list-pattern} has the shape $[\varphi_1,\ldots,\varphi_k]$,
          ...
          A \emph{One-Path Cartesian reachability rule of arity $k$} has the shape
...
To do so, we define a function $\mathit{flatten}$ from (potentially existentially-quantified) constrained list patterns
to matching logic patterns over a star-extended signature;
the idea behind this construction is that flattened (E)CLPs are matched
in star-extended models by lists of configurations, where each configuration from the list
matches the corresponding pattern from the (E)CLP.
We can then prove the following theorem, which says that one can reduce a CRL goal to a RL goal in a star-extended system.
...
by means of \Cref{thm:correspondence} and the existing proof system of reachability logic?''.
The answer is the following.
When one reduces a CRL goal into RL using \Cref{thm:correspondence},
