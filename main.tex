...
(that is, $s \not = \mathit{Cfg}^*$). TODO so what? I got lost there.
Similarly, we have
...
satisfying $\forall y \in \mathit{Var} \setminus \vec{X}.\, (\rho_0^\prime)^*(y) = \rho(\prime)$
and $\forall y \in \mathit{Var} \setminus \vec{Y}.\, (\rho_0^{\prime\prime})^*(y) = \rho(\prime)$,
and there exists some $l,j$ satisfying $1 \leq l \leq m$ and $1 \leq j \leq n$ such that
\begin{itemize}
    \item $(\mathcal{T}^*, \rho) \vDash P_l$; and
    \item $(\mathcal{T}^*, \rho) \vDash P_j$; and
