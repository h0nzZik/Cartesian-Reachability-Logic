...
\begin{enumerate}
    ...
    \end{enumerate}
    \end{proofenv}
    which is by \Cref{lem:mkListSemantics} equivalent to
    \begin{proofenv}
    exists $\rho_0 : \mathit{Var} \to \mathcal{T}^*$ such that $\rho_0(v) = \rho(v)$ for any $v \in \mathit{Var} \setminus \vec{Z}$, and exists $i_1 \in \{ 1, \ldots, m_1 \}, \ldots, i_k \in \{ 1, \ldots, m_k \}$ such that
    \begin{enumerate}
    ...
    \begin{proofenv}
        ...
        \end{enumerate}
    \end{proofenv}
    We let $\rho_1 := \rho_2$, where $\rho_2(v) := \rho_0(v)$
    for any $v$ of sort other then $\mathit{Cfg}^*$,
    let $i_j := j$, and have to prove that
    \begin{proofenv}
        \begin{enumerate}
            \item for any $v \in \mathit{Var} \setminus \vec{X}$, $\rho_2(v) = \rho(v)$;
            \item         
            \begin{equation*}
            (\mathcal{T}, c_j, \rho_2) \vDash \phi_{i_j} \, ;
            \end{equation*}
            \item and
            \begin{equation*}
            (\mathcal{T}, c_j, \rho_2) \vDash  P_{i_j} \land P \, .
            \end{equation*}
        \end{enumerate}
    \end{proofenv}
    We prove these three subgoals one by one.
    \begin{enumerate}
        \item Let $v \in \mathit{Var} \setminus \vec{X}$. If $v$ has sort $\mathit{Cfg}^*$,
              then surely $v \not\in \vec{Z}$, and therefore 
    \end{enumerate}
    %The first subgoal is trivial (note only that $\vec{X} \subseteq \vec{Z}$,
    %and therefore $(\mathit{Var} \setminus \vec{Z}) \subseteq (\mathit{Var} \setminus \vec{X})$).
