...
\begin{enumerate}
    ...
    \item A \emph{basic $\mathbf{\Sigma}$-pattern} of sort $s$
          is a term in $T_{\Sigma, s}(\mathit{Var})$,
...
\begin{enumerate}
    \item Let $((\Sigma, \mathit{Var}), \mathit{Cfg})$ be a reachability-logic signature.
          ...
          %\mathit{cfgdummy},
          \mathit{cfgheat} \}$\traian{You need to add cfgnil here}
    ...
    \item Let $S$ be a set of reachability rules over $(\mathbf{\Sigma}, \mathit{Cfg})$.
          ...
          by
          \begin{enumerate}
              \item defining a function $\mathit{heat}$ by\traian{maybe make clear that heat is mapping formul{\ae} to formul{\ae}}
              \begin{align*}
...
\begin{itemize}
    ...
    and ${\rho^*}^\prime(x) \in \mathcal{T}^*_s$\traian{you need to use $s'$ here and in the following.}
